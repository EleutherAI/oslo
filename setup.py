...
try:
    ...
    print("PyTorch could not import. Please install torch==1.1.0 to use OSLO.\nBuild exited with error: {}".format(err))
    sys.exit(-1)

if torch.__version__ != pytorch_min_version_str:
    print("OSLO requires PyTorch 1.11.0.\n"
           + "You are using torch.__version__ = {}.\nRequired version for torch.__version__ is {}.".format(torch.__version__, pytorch_min_version_str))
    sys.exit(-1)
...
if TORCH_MAJOR < 1 or (TORCH_MAJOR == 1 and TORCH_MINOR < 10):
    raise RuntimeError("OSLO requires PyTorch 1.11.0.\n" +
