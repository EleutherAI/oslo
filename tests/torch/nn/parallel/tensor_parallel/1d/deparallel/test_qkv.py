...
# [t][f*t]
weight_list = _TensorParallel1D._deconstruct_combined_qkv(
    weight_list, world_size, fusion_degree, dim
)
chunked_w = weight_list[rank].contiguous()
bias_list = _TensorParallel1D._deconstruct_combined_qkv(
    bias_list, world_size, fusion_degree, 0
)
chunked_b = bias_list[rank].contiguous()

linear_1d = RowParallelLinear(
...
recon_w = _TensorParallel1D._reconstruct_combined_qkv(
    recon_chunked_w, world_size, fusion_degree, dim
)
recon_b = _TensorParallel1D._reconstruct_combined_qkv(
    recon_chunked_b, world_size, fusion_degree, 0
)
