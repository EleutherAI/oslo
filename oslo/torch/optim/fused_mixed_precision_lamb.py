...
class FusedMixedPrecisionLamb(torch.optim.Optimizer):
    def __init__(
        ...
        # in order to facilitate device sync free execution
        defaults = dict(
        ...
        self.multi_tensor_l2norm = get_l2norm_mp_kernel().multi_tensor_l2norm_mp
        # Skip buffer
        self._dummy_overflow_buf = torch.tensor([0], dtype=torch.int, device=device)
        self.multi_tensor_lamb = get_lamb_mp_kernel().multi_tensor_lamb_mp
