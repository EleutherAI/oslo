...
class ParallelCachedEmbeddingBagTablewise(CachedEmbeddingBag):
    ...
    def __init__(
        ...
        self.rank_of_tables = [
            config.assigned_rank for config in embedding_bag_config_list
        ]
        self.global_table_num_embeddings_list = [
            config.num_embeddings for config in embedding_bag_config_list
        ]
        self.global_tables_num = len(embedding_bag_config_list)
        self.global_tables_offsets = torch.cumsum(
            torch.tensor([0] + self.global_table_num_embeddings_list), 0
        ).cuda()
        ...
        offset_cumsum = 0
        for table_i, table_num_embeddings in enumerate(
            self.global_table_num_embeddings_list
        ):
    ...
    def forward(
        ...
        if not already_split_along_rank:
            # not recommended. it takes time.
            batch_size = (offsets.shape[0]) // self.global_tables_num
            (
                local_indices,
                local_offsets,
                local_per_sample_weights,
            ) = self.split_along_rank(batch_size, indices, offsets, per_sample_weights)
        else:
            # recommended.
            batch_size = (offsets.shape[0]) // len(self.assigned_table_list)
            local_indices, local_offsets, local_per_sample_weights = (
                indices,
                offsets,
                per_sample_weights,
            )
        ...
        scatter_strides = [
            batch_size // self.world_size + int(i < remains)
            for i in range(self.world_size)
        ]
        output_full = dual_all_to_all_tablewise(
            local_output, self.pg, scatter_strides, self.embedding_dim_per_rank
        )
    ...
    def split_along_rank(
        ...
        """
        if input indices and offsets haven't been split along assigned rank, this function will do it.
        ...
        for i, handle_table in enumerate(self.assigned_table_list):
            ...
            if (not self.include_last_offset) and (
                batch_size * (handle_table + 1) >= indices.shape[0]
            ):
            ...
            local_indices_list.append(
                indices.narrow(
                    ...
                    indices_end_position - indices_start_position,
                ).sub(self.idx_offset_list[i])
            )
            # 2. local_offsets_list:
            if i + 1 == len(self.assigned_table_list):
                # till-the-end special case
                if not self.include_last_offset:
                    local_offsets = offsets.narrow(
                        0, batch_size * handle_table, batch_size
                    ).add(offset_pre_end - offsets[batch_size * (handle_table)])
                else:
                    local_offsets = offsets.narrow(
                        0, batch_size * handle_table, batch_size + 1
                    ).add(offset_pre_end - offsets[batch_size * (handle_table)])
                local_offsets_list.append(local_offsets)
            else:
                local_offsets = offsets.narrow(
            ...
            if per_sample_weights != None:
                local_per_sample_weights_list.append(
                    per_sample_weights[indices_start_position:indices_end_position]
                )
