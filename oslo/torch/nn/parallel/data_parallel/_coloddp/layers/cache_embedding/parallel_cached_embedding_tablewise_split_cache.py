...
from oslo.torch.nn.parallel.data_parallel._coloddp.layers.embedding_config import (
            ...
            with record_function("(tablewise) prepare indices and offsets"):
                ...
                with record_function("part 2"):
                    ...
                    if self.include_last_offset:
                        ...
                        ).sub(offsets[batch_size * (handle_table)])
                    else:
                ...
                if per_sample_weights != None:
                    local_per_sample_weights = per_sample_weights[
                        indices_start_position:indices_end_position
                    ]
            with record_function("(tablewise) tablewise forward"):
                local_output_list.append(
                    self.cached_embedding_bag_list[i](
    ...
    def print_comm_stats_(self):
        ...
        for cached_embedding_bag in self.cached_embedding_bag_list:
            cuda_to_cpu_elem_num += (
                cached_embedding_bag.cache_weight_mgr._cuda_to_cpu_numel
            )
            cpu_to_cuda_elem_num += (
                cached_embedding_bag.cache_weight_mgr._cpu_to_cuda_numel
            )
