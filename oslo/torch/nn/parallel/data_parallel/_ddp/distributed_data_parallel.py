...
def DistributedDataParallel(
    ...
    add_wrapper(module, mode=ParallelMode.DATA, wrapper=ddp, parallel_context=parallel_context)
...
class _DDPJoinHook(JoinHook):
    def __init__(self, ddp, divide_by_initial_world_size):
        ...
        """
        assert isinstance(ddp, _DistributedDataParallel), (
...
class _DistributedDataParallel(Module, Joinable):
    ...
    to construct this module:
        ...
        >>> )
        >>> model = _DistributedDataParallel(model, device_ids=[i], output_device=i)
    ...
    Args:
        ...
        static_graph (bool): When set to ``True``, DDP knows the trained graph is
                     ...
                     Example::
                         >>> model_DDP = torch.nn.parallel._DistributedDataParallel(model)
                         >>> # Training loop
                         >>> .....
    ...
    Example::
        ...
        >>> net = torch.nn.parallel._DistributedDataParallel(model, pg)
    """
    ...
    def __init__(
        ...
        super(_DistributedDataParallel, self).__init__()
    ...
    def __setstate__(self, state):
        ...
        self.process_group = _get_default_group()
        super(_DistributedDataParallel, self).__setstate__(state)
    ...
    def no_sync(self):
        ...
        Example::
            >>> ddp = torch.nn.parallel._DistributedDataParallel(model, pg)
            >>> with ddp.no_sync():
            >>>   for input in inputs:
    ...
    def forward(self, *inputs, **kwargs):
        with torch.autograd.profiler.record_function("DistributedDataParallel.forward"):
            ...
            if self.device_ids:
                ...
                output = self.module_forward(*inputs[0], **kwargs[0])
            else:
                output = self.module_forward(*inputs, **kwargs)
    ...
    def join(
        ...
        Example::
            ...
            >>>     model = torch.nn.parallel._DistributedDataParallel(
