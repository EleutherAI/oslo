...
class TransformDistributedSpec(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx: torch.autograd.function.FunctionContext, tensor: torch.Tensor, old_dist_spec: DistributedSpec, dist_spec: DistributedSpec, parallel_context: ParallelContext, forward_trans_func: Callable, backward_trans_func: Callable) -> torch.Tensor:
    ...
    def backward(ctx: torch.autograd.function.FunctionContext, grad_outputs):
        return ctx.backward_trans_func(grad_outputs, ctx.dist_spec, ctx.old_dist_spec,
                                       ctx.parallel_context), None, None, None, None, None
...
class DistributedSpecManager:
    ...
    @staticmethod
    def _sanity_check(old_dist_spec: DistributedSpec, dist_spec: DistributedSpec) -> None:
        pass

    @staticmethod
    def _shard_as(tensor: torch.Tensor, old_dist_spec: DistributedSpec, dist_spec: DistributedSpec,
        ...
        """
        assert old_dist_spec.placement.value == 'r', f"The old_dist_spec of DistributedSpecManager._shard_as must be REPLICATE!"
    ...
    @staticmethod
    def _gather(tensor: torch.Tensor, old_dist_spec: DistributedSpec, parallel_context: ParallelContext) -> torch.Tensor:
        ...
        """
        assert old_dist_spec.placement.value == 's', f"The old_dist_spec of DistributedSpecManager._gather must be SHARD!"
        is_cpu_tensor = False
        ...
        buffer = [torch.empty_like(tensor) for _ in range(parallel_context.get_world_size(ParallelMode.TENSOR))]
        assert tensor.device.type == 'cuda'
        dist.all_gather(buffer, tensor, group=parallel_context.get_group(ParallelMode.TENSOR))
    ...
    @staticmethod
    def _all_to_all(tensor: torch.Tensor, old_dist_spec: DistributedSpec, dist_spec: DistributedSpec,
                    parallel_context: ParallelContext) -> torch.Tensor:
        """
        ...
        dist.all_to_all(gather_list, scatter_list, group=parallel_context.get_group(ParallelMode.TENSOR))
    ...
    @staticmethod
    def _r2r(tensor: torch.Tensor, old_dist_spec: DistributedSpec, dist_spec: DistributedSpec, parallel_context: ParallelContext) -> torch.Tensor:
        """
    ...
    @staticmethod
    def _r2s(tensor: torch.Tensor, old_dist_spec: DistributedSpec, dist_spec: DistributedSpec, parallel_context: ParallelContext) -> torch.Tensor:
        """
        ...
        """
        DistributedSpecManager._sanity_check(old_dist_spec, dist_spec)
        return DistributedSpecManager._shard_as(tensor, old_dist_spec, dist_spec, parallel_context)

    @staticmethod
    def _s2r(tensor: torch.Tensor, old_dist_spec: DistributedSpec, dist_spec: DistributedSpec, parallel_context: ParallelContext) -> torch.Tensor:
        """
    ...
    @staticmethod
    def _s2s(tensor: torch.Tensor, old_dist_spec: DistributedSpec, dist_spec: DistributedSpec, parallel_context: ParallelContext) -> torch.Tensor:
        """
        ...
        if len(old_dist_spec.dims) == 1 and len(dist_spec.dims) == 1:
            ...
            return DistributedSpecManager._all_to_all(tensor, old_dist_spec, dist_spec, parallel_context)
        tensor = DistributedSpecManager._gather(tensor, old_dist_spec, parallel_context)
        return DistributedSpecManager._shard_as(tensor, old_dist_spec, dist_spec, parallel_context)

    @staticmethod
    def handle_trans_spec(tensor: torch.Tensor, old_dist_spec: DistributedSpec, dist_spec: DistributedSpec,
                          parallel_context: ParallelContext) -> torch.Tensor:
        """
        ...
        Returns:
            torch.Tensor: The transformed tensor with the new distributed specification.
        """
        assert isinstance(old_dist_spec, DistributedSpec), f"{type(old_dist_spec)} should be DistributedSpec"
        assert isinstance(dist_spec, DistributedSpec), f"{type(dist_spec)} should be DistributedSpec"
        forward_trans_handle = getattr(DistributedSpecManager, f'_{old_dist_spec.placement.value}2{dist_spec.placement.value}')
        if not DistributedSpecManager._use_autograd_function:
            return forward_trans_handle(tensor, old_dist_spec, dist_spec, parallel_context)
        backward_trans_handle = getattr(DistributedSpecManager,
                                        f'_{dist_spec.placement.value}2{old_dist_spec.placement.value}')
        return TransformDistributedSpec.apply(tensor, old_dist_spec, dist_spec, parallel_context, forward_trans_handle,
