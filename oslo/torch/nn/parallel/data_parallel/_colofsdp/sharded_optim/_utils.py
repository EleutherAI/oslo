...
def reduce_tensor(tensor, dtype, dst_rank=None, parallel_mode=ParallelMode.DATA):
    ...
    world_size = ParallelContext.get_world_size(parallel_mode)
    group = ParallelContext.get_group(parallel_mode)
    ...
    if use_all_reduce:
        ...
        ranks_in_group = ParallelContext.get_ranks_in_group(parallel_mode)
    ...
    if tensor.dtype != dtype and tensor is not tensor_to_reduce:
        local_rank = ParallelContext.get_local_rank(parallel_mode)
