...
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.common_op_utils import (
    _register_default_op,
)
...
def _custom_partial_tensor_op(func):
    ...
    """
    return functools.partial(_decorator_func, op=func, op_table=_PARTIAL_TENSOR_OPS)
...
class _PartialTensor(torch.Tensor):
    ...
    def __new__(
        cls, local_shard, process_group=None, reduce_op=distributed_c10d.ReduceOp.SUM
    ):
    ...
    def reshard(self, resharding_spec: shard_spec.ShardingSpec) -> "ShardedTensor":
        ...
        chunk_mode_res = (
            self._local_shard.size(sharding_dim) % self._process_group.size()
        )
...
def _transpose_impl(types, args=(), kwargs=None, process_group=None):
    ...
    return _PartialTensor(
        torch.transpose(input, dim0, dim1), process_group, partial_tensor._reduce_op
    )
