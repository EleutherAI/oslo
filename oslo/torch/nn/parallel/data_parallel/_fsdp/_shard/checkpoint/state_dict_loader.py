...
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.checkpoint.api import (
    CheckpointException,
)
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.checkpoint.metadata import (
    BytesReadRequest,
...
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.checkpoint.resharding import (
    _prepare_sharded_tensor_read,
    _shards_get_overlap_region_wrt_saved_tensor,
)
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.checkpoint.storage import (
    StorageReader,
)
        ...
        if isinstance(obj, ShardedTensor):
            ...
            if isinstance(md, BytesStorageMetadata):
                ...
                brr = BytesReadRequest(
                    bytes=bytes_io, storage_key=md.storage_key, fqn=fqn
                )
...
def load_state_dict(
    ...
    if not no_dist:
        ...
        dist.all_gather_object(object_list=all_errors, obj=result, group=process_group)

        node_failures = cast(
            Dict[int, BaseException],
            {i: err for i, err in enumerate(all_errors) if err is not None},
        )
        if len(node_failures) > 0:
            global_result = CheckpointException(
                "failed to read checkpoint", node_failures
            )
    elif result is not None:
        global_result = CheckpointException(
            "failed to read storage", {coordinator_rank: result}
        )
...
def validate_metadata(state_dict: Dict[str, Any], metadata: Metadata) -> None:
    """
    ...
    for fqn, obj in state_dict.items():
        if isinstance(obj, ShardedTensor):
            ...
            if not isinstance(md, ShardedTensorStorageMetadata):
                raise ValueError(
                    f"{fqn}: Expected ShardedTensorStorageMetadata but found: {type(md)}"
                )
            ...
            if not isinstance(md, TensorStorageMetadata):
                raise ValueError(
                    f"{fqn}: Expected TensorStorageMetadata but found: {type(md)}"
                )
