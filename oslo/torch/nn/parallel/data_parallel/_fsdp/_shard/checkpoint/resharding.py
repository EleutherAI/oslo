...
def _create_storage_key(storage_key_to_fqn: Dict[str, str], fqn: str) -> str:
    """
...
def _compute_sharded_tensor_md(
    tensor: ShardedTensor, shard_to_storage_key: Dict[str, str]
) -> ShardedTensorStorageMetadata:
    smd = []
...
def _get_shard_storage_key(
    tensor_storage_key: str, shard, storage_key_to_fqn: Dict[str, str]
) -> str:
...
def _prepare_sharded_tensor_write(
    sharded_tensor: ShardedTensor, storage_key: str, storage_key_to_fqn: Dict[str, str]
    ...
    for shard_md in sharded_tensor.metadata().shards_metadata:
        shard_storage_key = _get_shard_storage_key(
            storage_key, shard_md, storage_key_to_fqn
        )
...
def _compute_tensor_md(storage_key: str, tensor: Tensor) -> TensorStorageMetadata:
    return TensorStorageMetadata(storage_key=storage_key, size=tensor.size())
...
def _compute_bytes_md(storage_key: str, bytes: io.BytesIO) -> BytesStorageMetadata:
    return BytesStorageMetadata(storage_key=storage_key, length=len(bytes.getbuffer()))
