...
def _prepare(
    ...
    for fqn, obj in state_dict.items():
        if isinstance(obj, ShardedTensor):
            st_write_reqs, st_md = _prepare_sharded_tensor_write(
                obj, fqn, storage_key_to_fqn
            )
            ...
            byte_write_reqs, bytes_md = _prepare_bytes_write(
                bytes_io, fqn, storage_key_to_fqn
            )
            if write_replicated_data:
...
def save_state_dict(
    ...
    if not no_dist:
        dist.broadcast_object_list(
            exceptions, group=process_group, src=coordinator_rank
        )

    if exceptions[0] is not None:
        raise CheckpointException(
            "failed to prepare storage", {coordinator_rank: exceptions[0]}
        )
    ...
    if not no_dist:
        dist.broadcast_object_list(result, group=process_group, src=coordinator_rank)
