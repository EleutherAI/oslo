...
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.sharded_tensor.api import (
    ShardedTensor,
)
...
class ReplicatedTensor(torch.Tensor):
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        ...
        with torch._C.DisableTorchFunction():
            ...
            result_not_replicated = isinstance(rs, torch.Tensor) and not isinstance(
                rs, ReplicatedTensor
            )
            should_convert_to_replicated = all_replicated or (
                replicated_with_non_tensor
                and func in _REPLICATED_WITH_NON_TENSOR_ALLOWLIST
            )
