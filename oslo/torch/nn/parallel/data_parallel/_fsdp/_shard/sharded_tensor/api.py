...
import oslo.torch.nn.parallel.data_parallel._fsdp._shard.sharding_spec as shard_spec
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.metadata import ShardMetadata
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.sharded_tensor.metadata import (
    TensorProperties,
    ShardedTensorMetadata,
)
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.sharded_tensor.reshard import (
    reshuffle_local_shard,
    reshard_local_shard,
)
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.sharded_tensor.shard import Shard
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.sharded_tensor.utils import (
...
class ShardedTensor(object):
    ...
    def _init_from_local_shards(
        ...
        *global_size,
        ...
        sharded_tensor._sharding_spec = (
            shard_spec._infer_sharding_spec_from_shards_metadata(
    ...
    def _init_from_local_shards_and_global_metadata(
        ...
        for shard in local_shards:
            ...
            assert (
                shard_meta in local_shard_metadatas
            ...
            _raise_if_mismatch(
                local_device, local_shard_tensor.device, "device", current_rank
            )
            _raise_if_mismatch(
        ...
        if sharding_spec is None:
            sharded_tensor._sharding_spec = (
                shard_spec._infer_sharding_spec_from_shards_metadata(shards_metadata)
            )
        else:
    ...
    def contiguous(self) -> ShardedTensor:
        ...
        for shard in self.local_shards():
            local_shards.append(Shard(shard.tensor.contiguous(), shard.metadata))
        return ShardedTensor._init_from_local_shards_and_global_metadata(
            local_shards,
            self._metadata,
    ...
    def transpose(self, dim0, dim1) -> ShardedTensor:
        ...
        """
        return handle_torch_function(
            torch.Tensor.transpose, (self, dim0, dim1), self, dim0, dim1
        )
    ...
    def bmm(self, st2, *, out=None) -> ShardedTensor:
        ...
        """
        return handle_torch_function(
            torch.Tensor.bmm, (self, st2, out), self, st2, out=out
        )
    ...
    def chunk(self, chunks, dim=0) -> List[ShardedTensor]:
        ...
        """
        return handle_torch_function(
            torch.Tensor.chunk, (self, chunks, dim), self, chunks, dim=dim
        )
    ...
    def requires_grad_(self, requires_grad=True):
        return handle_torch_function(
            torch.Tensor.requires_grad_, (self, requires_grad), self, requires_grad
        )
    ...
    def __deepcopy__(self, memo):
        return handle_torch_function(
            torch.Tensor.__deepcopy__, (self, memo), self, memo
        )

    def clone(self, *, memory_format=torch.preserve_format):
        return handle_torch_function(
            torch.Tensor.clone, (self,), self, memory_format=memory_format
        )
