...
import oslo.torch.nn.parallel.data_parallel._fsdp.fully_sharded_data_parallel as FSDP
from oslo.torch.nn.parallel.data_parallel._fsdp import FlatParameter
...
def _unflatten_optim_state(
    ...
    """
    assert (
        sum(p is flat_param for p in fsdp_module.params) == 1
...
def _flatten_full_optim_state_dict(
    ...
    for flat_param_id, param in enumerate(flat_param_id_to_param):  # type: ignore[assignment]
        ...
        # For parameters from non-FSDP modules, we do not need to flatten
            ...
            flat_osd_state[flat_param_id] = copy.copy(full_osd_state[unflat_param_name])
            unflat_param_names_to_flat_param_id[unflat_param_name] = flat_param_id
...
def _flatten_tensor_optim_state(
    ...
    assert flat_tensor.shape[0] <= full_padded_shape[0], (
        f"tensor optim state: {flat_tensor.shape} " f"parameter: {full_padded_shape}"
    )
    return flat_tensor
    ...
    for param_id, param_state in flat_osd["state"].items():
        ...
        for state_name, state_value in param_state.items():
            is_pos_dim_tensor_state = (
                torch.is_tensor(state_value) and state_value.dim() > 0
            )
            ...
            if param_id in fsdp_flat_param_ids:  # FSDP parameter
                ...
                chunk, num_to_pad = FSDP._FullyShardedDataParallel._get_chunk(
                    state_value,
                    0,
                    world_size,
                )
...
def _broadcast_processed_optim_state_dict(
    ...
    obj_list = (
        [processed_optim_state_dict, fsdp_flat_param_ids] if rank == 0 else [None, None]
    )
...
def _broadcast_sharded_pos_dim_tensor_state(
    ...
    if rank == 0:
        ...
        get_shard = functools.partial(
            FSDP._FullyShardedDataParallel._get_shard_functional,
            unsharded_tensor,
        )
...
def _get_flat_param_to_fsdp_module(model: torch.nn.Module):
    ...
    for module in model.modules():
        if isinstance(module, FSDP._FullyShardedDataParallel):
            module._lazy_init()
