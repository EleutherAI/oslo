...
from oslo.torch.nn.parallel.data_parallel._fsdp._optim_utils import (
...
from oslo.torch.nn.parallel.data_parallel._fsdp._shard.sharded_tensor import (
    Shard,
    ShardedTensor,
    init_from_local_shards,
)
from oslo.torch.nn.parallel.data_parallel._fsdp._utils import (
    _apply_to_modules,
    _apply_to_tensors,
...
from oslo.torch.nn.parallel.data_parallel._fsdp.wrap import (
    _recursive_wrap,
...
class FullStateDictConfig(StateDictConfig):
    ...
    context manager as follows:
        ...
        >>> with _FullyShardedDataParallel.state_dict_type(fsdp, StateDictType.FULL_STATE_DICT, cfg):
...
class _ExecOrderData:
    ...
    def init(self, root_module: "_FullyShardedDataParallel"):
        assert root_module._is_root, (
            "This data structure should only be " "initialized on an FSDP root module"
        )
    ...
    def get_unflat_param_names(self, param_index: int) -> List[str]:
        ...
        if param is None:
            return []
        assert (
            param in self._param_to_unflat_param_names
...
def FullyShardedDataParallel(
    ...
    if transformer_wrap_layers is not None:
        ...
        transformer_wrap_layers_str = [layer.__name__ for layer in transformer_wrap_layers]
...
class _FullyShardedDataParallel(nn.Module):
    ...
    def __init__(
        ...
        ignored_params, ignored_param_names = self._get_ignored_params(
        ...
        if auto_wrap_policy is not None:
            self._check_wrapped(
                module,
                check_fn=lambda mod: not isinstance(mod, _FullyShardedDataParallel),
            ...
            _recursive_wrap(
                ...
                wrapper_cls=_FullyShardedDataParallel,
        ...
        if is_meta_module:
            ...
            assert (
                _TORCHDISTX_AVAIL
            ), "Got torchdistX initialized module but torchdistX lib is not available."
            if param_init_fn is not None:
                _run_param_init_fn()
            else:
                # Call default torchdistX initialization function. Omit re-initialization of FSDP submodules
                # which is unnecessary.
                check_fn = lambda k: not isinstance(
                    k, _FullyShardedDataParallel
                )  # noqa: E731
    ...
    def _get_ignored_modules(
        ...
        for module in ignored_root_modules:
            ...
            if isinstance(module, _FullyShardedDataParallel):
        ...
        ignored_modules = set(
            ...
            if not isinstance(child, _FullyShardedDataParallel)
    ...
    def _get_buffer_names(self, root_module: torch.nn.Module) -> Set[str]:
        ...
        def module_fn(module, prefix, buffer_names):
            ...
            if not isinstance(module, _FullyShardedDataParallel):
    ...
    def fsdp_modules(
    ...
    ) -> List["_FullyShardedDataParallel"]:
        """
        ...
        Returns:
            List[_FullyShardedDataParallel]: FSDP modules that are nested in
            the input ``module``.
        """
        return [
            submodule
            for submodule in module.modules()
            if isinstance(submodule, _FullyShardedDataParallel)
            and (not root_only or submodule.check_is_root())
        ]

    def apply(self, fn: Callable[[nn.Module], None]) -> "_FullyShardedDataParallel":
    ...
    def _cast_buffers(
        ...
        for module in self.modules():
            if (
                module is not self
                and isinstance(module, _FullyShardedDataParallel)
                and recurse
            ):
    ...
    def _get_chunk(
        ...
        assert num_to_pad >= 0, "Chunk's size should at most the first chunk's size"
    ...
    def _get_shard_functional(
        ...
        chunk, num_to_pad = _FullyShardedDataParallel._get_chunk(
            tensor,
            rank,
            world_size,
        )
    ...
    def _get_shard(
        ...
        return _FullyShardedDataParallel._get_shard_functional(
            tensor,
            rank,
            self.world_size,
        )
    ...
    def _set_is_root(self) -> None:
        ...
        for m in self.modules():
            if m is not self and isinstance(m, _FullyShardedDataParallel):
    ...
    def state_dict_type(
        ...
        for submodule in _FullyShardedDataParallel.fsdp_modules(module):
    ...
    def _full_post_state_dict_hook(
        ...
        for key in state_dict:
            ...
            if clean_key not in self._ignored_param_names and not getattr(
    ...
    def _post_state_dict_hook(
        ...
        *args: Any,
        ...
        """
        self = cast(_FullyShardedDataParallel, module)
        processed_state_dict = self._post_state_dict_hook_fn[self._state_dict_type](
            state_dict, prefix
        )
    ...
    def state_dict(self, *args, **kwargs):
        ...
        if self._state_dict_type == StateDictType.FULL_STATE_DICT:
            ...
            if (
                self.module.flat_param is not None
                and not self.module.flat_param._is_sharded
            ):
                raise RuntimeError(
    ...
    def _full_pre_load_state_dict_hook(
        ...
        self._full_param_ctx = self._summon_full_params(recurse=False, writeback=True)
    ...
    def _local_pre_load_state_dict_hook(
        ...
        if fqn not in state_dict:
            assert (
                getattr(self.module, FLAT_PARAM, None) is None
        ...
        with torch.autograd.profiler.record_function(
            ...
            if self._is_root:
                ...
                args, kwargs = _to_kwargs(
                    args, kwargs, self.compute_device.index, False
                )
                args = args[0]
                kwargs = kwargs[0]

            # Cast inputs to their mixed precision type.
            if self._is_root and self._mixed_precision_enabled_for_params():
                                ...
                                if p._is_sharded:
                                    with torch.no_grad():
                                        ...
                                        for p, (full_param, _) in zip(
                                            ...
                                            full_param = full_param.to(
                                                self.compute_device
                                            )
                                            self._update_p_data(
                                                p,
                                                output_tensor=full_param,
                                            )
    ...
    def summon_full_params(
        ...
        fsdp_modules = _FullyShardedDataParallel.fsdp_modules(module, root_only=True)
    ...
    def _post_backward_hook(self, param: Parameter, *unused: Any) -> None:
        ...
        with torch.cuda.stream(self._streams["post_backward"]):
            ...
            if param._is_sharded:  # type: ignore[attr-defined]
                ...
                assert (
                    self.world_size == 1
                    or self.sharding_strategy == ShardingStrategy.NO_SHARD
    ...
    def _wait_for_post_backward(self) -> None:
        ...
        def _finalize_params(fsdp_module: _FullyShardedDataParallel) -> None:
            ...
            for p in fsdp_module.params:
                if p.requires_grad:
                    ...
                    if hasattr(p, "_cpu_grad"):
                        ...
                        p_assert(
                            not p._is_sharded,
                            "All sharded parameters should " "use `_saved_grad_shard`",
                        )
        ...
        for m in self.modules():  # includes self
            if isinstance(m, _FullyShardedDataParallel):
                _finalize_params(m)
            ...
            for p in self.params:
                ...
                # If full param has been rebuilt or has not been freed, no need to call all gather
                    ...
                    output_tensors.append(
                        (p._full_param_padded, self.reshard_after_forward)
                    )
    ...
    def _check_rebuild_full_params(self, param: FlatParameter):
        ...
        if not eod.is_first_iter:
            ...
            if eod.index >= len(eod.param_order):
                msg_prefix = (
                    "Expected to not rebuild any more parameters "
                ...
                if param_index != expected_param_index:
                    expected_param_names = eod.get_unflat_param_names(
                        expected_param_index
                    )
                    assert (
                        len(expected_param_names) > 0
                    ), "Expected parameter should always be valid"
                    msg_prefix = (
                        "Expected to rebuild parameters in "
            ...
            for (r1, i1), (r2, i2) in itertools.combinations(
                ((rank, indices[rank]) for rank in range(self.world_size)),
                2,
            ):
    ...
    def full_optim_state_dict(
        ...
        group = (
            model.process_group if hasattr(model, "process_group") else None
        )  # not all `torch.nn.Module`s have `process_group`
        ...
        for flat_param_id, param in enumerate(flat_param_id_to_param):  # type: ignore[assignment]
            ...
            if isinstance(param, FlatParameter):
                ...
                unflat_state = _unflatten_optim_state(
                    flat_param_to_fsdp_module[param],
                    param,
                ...
                if to_save:
                    assert (
                        len(unflat_state) == len(unflat_param_names)
                        and len(unflat_state) == param._num_unflattened_params
                    ), (
    ...
    def scatter_full_optim_state_dict(
        ...
        processed_osd, fsdp_flat_param_ids = _broadcast_processed_optim_state_dict(
            processed_osd if rank == 0 else None,
            fsdp_flat_param_ids if rank == 0 else None,
            rank,
            group,
        )
...
def _calc_grad_norm(parameters: List[torch.nn.Parameter], p: float) -> torch.Tensor:
    ...
    if p == math.inf:
        local_norm = torch.tensor(
            max(par.grad.detach().abs().max() for par in parameters)
        )
    else:
