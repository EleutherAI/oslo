...
def sharded_op_impl(func):
    ...
    can be used to override existing DistributedTensor operators or write a new
    one not supported by DistributedTensor. If the operator in question is covered
    by ``__torch_function__`` dispatch and has a DistributedTensor as any of its
    ...
    Example:
        ...
        >>> weight = DistributedTensor(torch.rand(32, 16))
        >>> bias = DistributedTensor(torch.rand(16))
