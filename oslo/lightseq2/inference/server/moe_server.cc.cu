...
/**
@file
moe server based on tensorrt inference server.
*/
...
private:
  ...
  lightseq::cuda::MoeWeight<OPTYPE> tw_;
  std::shared_ptr<lightseq::cuda::MoeDecoder<OPTYPE>> decoder_;
  std::shared_ptr<lightseq::cuda::MoeEncoder<OPTYPE>> encoder_;
};
...
Context::Context(const std::string &instance_name,
    ...
    : instance_name_(instance_name), model_config_(model_config),
      ...
      d_input_(nullptr), d_input_copy_(nullptr), d_padding_mask_(nullptr),
      d_encoder_output_(nullptr), d_buf_(nullptr), d_output_(nullptr),
      d_src_lang_id_(nullptr), d_trg_lang_id_(nullptr), stream_(nullptr),
      hd_(nullptr) {}
...
int Context::Init() {
  ...
  std::string weight_path = model_path + "/" + "moe.pb";
  std::ifstream fproto(weight_path.c_str());
  if (!fproto.good()) {
    weight_path = model_path + "/" + "moe.hdf5";
    std::ifstream fhdf5(weight_path.c_str());
    if (!fhdf5.good()) {
      LOG_ERROR << "Neither moe.pb nor moe.hdf5 "
  ...
  std::string res = tw_.initializing(weight_path, max_batch_size);
  if (!res.empty()) {
  ...
  int err;
  err = AllocateCudaBuffers(&d_input_, max_batch_size * tw_._max_step *
                                           datatype_bytesize_);
  if (err != kSuccess) {
    return err;
  }
  err = AllocateCudaBuffers(&d_input_copy_, max_batch_size * tw_._max_step *
                                                datatype_bytesize_);
  if (err != kSuccess) {
    return err;
  }
  err = AllocateCudaBuffers(&d_padding_mask_, max_batch_size * tw_._max_step *
                                                  datatype_bytesize_);
  if (err != kSuccess) {
    return err;
  }
  ...
  err = AllocateCudaBuffers(&d_output_, max_batch_size * tw_._max_step *
                                            datatype_bytesize_);
  if (err != kSuccess) {
    return err;
  }
  ...
  encoder_ = std::make_shared<lightseq::cuda::MoeEncoder<OPTYPE>>(
  ...
  decoder_ = std::make_shared<lightseq::cuda::MoeDecoder<OPTYPE>>(
  ...
  cuerr = cudaStreamSynchronize(stream_);
  if (cuerr != cudaSuccess) {
    LOG_ERROR << "failed to init GPU for moe: " << cudaGetErrorString(cuerr)
              << std::endl;
    return kCudaExecute;
  }
  LOG_INFO << "moe, release-version[" << __DATE__ << " " << __TIME__
...
int Context::GetInputTensorGPU(CustomGetNextInputFn_t input_fn,
  ...
  while (true) {
    ...
    if (cuerr != cudaSuccess) {
      LOG_ERROR << "failed to copy input values to GPU for moe: "
                << cudaGetErrorString(cuerr) << std::endl;
...
int Context::ExecuteGPU(const uint32_t payload_cnt, CustomPayload *payloads,
  ...
  for (uint32_t pidx = 0; pidx < payload_cnt; ++pidx) {
    ...
    if (cuerr != cudaSuccess) {
      LOG_ERROR << "failed to copy output values from GPU for moe: "
                << cudaGetErrorString(cuerr) << std::endl;
  ...
  if (cuerr != cudaSuccess) {
    LOG_ERROR << "failed to synchronize GPU for moe: "
              << cudaGetErrorString(cuerr) << std::endl;
    return kCudaExecute;
  }
  return kSuccess;
...
const char *CustomErrorString(void *custom_context, int errcode) {
  ...
  case kWeightLoad:
    return "load moe weight in .pb failed";
  case kModelSize:
    return "inappropriate moe model size";
  default:
    break;
  }
