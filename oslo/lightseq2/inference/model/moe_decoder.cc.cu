...
MoeDecoder<OpType_>::MoeDecoder(int max_batch_size, const int *p_d_padding_mask,
    ...
    : _max_batch_size(max_batch_size), _max_thread_per_block(1024),
      ...
      _gate_weight_offset(0), _p_d_dec_gate_wei(tw.get_dec_gate_wei()),
...
*/
template <OperationType OpType_> std::string MoeDecoder<OpType_>::check() {
...
Decode one step
*/
template <OperationType OpType_> bool MoeDecoder<OpType_>::run_step() {
  embedding();
  decoder_stack();
...
Decode embedding
*/
template <OperationType OpType_> void MoeDecoder<OpType_>::embedding() {
...
Decoder feedforward, composed by self_atten,
  enc-dec-atten, ffn
*/
template <OperationType OpType_> void MoeDecoder<OpType_>::decoder_stack() {
...
*/
template <OperationType OpType_> void MoeDecoder<OpType_>::self_attention() {
...
*/
template <OperationType OpType_> void MoeDecoder<OpType_>::encdec_attention() {
  /* ---step 0. layer_norm, add output_bias to "query"--- */
  ker_norm_layer_resual_launcher<_DataType>(
      _step_token_num, _tw._hidden_size, _stream, _p_d_cur_step_query,
...
template <OperationType OpType_> void MoeDecoder<OpType_>::ffn_add_norm() {
...
template <OperationType OpType_> void MoeDecoder<OpType_>::ffn() {
  /* ---step 0. layer_norm, add output_bias to "query"--- */
  ker_norm_layer_resual_launcher<_DataType>(
      _step_token_num, _tw._hidden_size, _stream, _p_d_cur_step_query,
...
template <OperationType OpType_> void MoeDecoder<OpType_>::moe_fw_hard_gate() {
  ...
  if (_batch_size == 1) {
    ...
    */
    ...
    for (auto it = _gate_sets->begin(); it != _gate_sets->end(); it++) {
      ...
      // _p_d_moe_input_buf_tmp: [beam_size*cur_gate_size , hidden_dim]
      // pointer of _p_d_moe_input_buf_tmp each gate will accumulate for each
      // gate sequence
...
template <OperationType OpType_> void MoeDecoder<OpType_>::moe_fw() {
  ker_norm_layer_prepost_launcher<_DataType>(
      _step_token_num, _tw._hidden_size, _stream, _p_d_cur_step_query,
      _p_d_query_buf1, _p_d_dec_wei[_weight_offset + 12],
      _p_d_dec_wei[_weight_offset + 13], _max_thread_per_block,
...
template <OperationType OpType_> bool MoeDecoder<OpType_>::sample() {
  CHECK_GPU_ERROR(
...
template <OperationType OpType_> bool MoeDecoder<OpType_>::beam_search() {
  /*
