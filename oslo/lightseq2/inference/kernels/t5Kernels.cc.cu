...
template <>
__global__ void
t5_ker_norm_layer<__half>(__half *matrix, __half *out, const __half *scale,
                          const __half *bias, int half_hidden_size) {
...
template <typename T>
__global__ void
t5_ker_correlation_softmax_encself(T *correlation, const int *src_padding_mask,
                                   int batch_seq_len, const T *pos_emb,
                                   int head_num) {
...
template void
ker_gelu_first_elementmul_launcher<float>(int batch_token_num, int block_dim,
