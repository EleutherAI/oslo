...
MT5::MT5(const std::string weight_path, const int max_batch_size)
  ...
  */
  ...
  encoder_ = std::make_shared<MT5Encoder<mt5_optype>>(
  ...
  std::cout << "Allocated " << buf_bytesize / (1024 * 1024)
            << "MB GPU buffer for mt5" << std::endl;
...
const int *MT5::get_result_ptr() { return d_output_; }

const float *MT5::get_score_ptr() { return decoder_->_p_d_alive_seq_score; }

int MT5::get_output_seq_len() { return decoder_->_cur_step + 1; };
...
void MT5::set_input_ptr(int index, void *input_ptr) {
  switch (index) {
    case 0:
...
void MT5::set_output_ptr(int index, void *output_ptr) {
  switch (index) {
    case 0:
...
const void *MT5::get_output_ptr(int index) {
  switch (index) {
    case 0:
...
std::vector<int> MT5::get_input_max_shape(int index) {
  switch (index) {
    case 0:
...
std::vector<int> MT5::get_output_max_shape(int index) {
  switch (index) {
    case 0:
...
DataType MT5::get_input_dtype(int index) {
  switch (index) {
    case 0:
...
DataType MT5::get_output_dtype(int index) {
  switch (index) {
    case 0:
      return DataType::kInt32;
      break;
