...
*/
template <> float MT5Weight<OperationType::FP32>::float2required(float value) {
  return value;
}
...
*/
template <> __half MT5Weight<OperationType::FP16>::float2required(float value) {
...
*/
template <OperationType OpType_>
void MT5Weight<OpType_>::hdf5_parse_enc_wei(hid_t hdf5_file) {
  size_t value_size =
...
*/
template <OperationType OpType_>
void MT5Weight<OpType_>::hdf5_parse_dec_wei(hid_t hdf5_file) {
  size_t value_size =
      (_hidden_size * 2 + _hidden_size * _hidden_size * 3 + _hidden_size * 3 +
       _hidden_size * _hidden_size + _hidden_size * 3 +
...
std::string MT5Weight<OpType_>::initializing(std::string weight_path,
  ...
  if (endswith(weight_path, ".pb")) {
    return "MT5 model does not support protocol buffer(due to larger than "
           "2GB), please use hdf5 instead.";
...
template class MT5Weight<OperationType::FP16>;
template class MT5Weight<OperationType::FP32>;
