...
class MoeWeight {
 private:
  ...
  void proto_get_model_config(const Moe &moe, bool only_decoder = false);
  std::string proto_parse_emb_wei(const MoeEmbeddingLayer &layer,
                                  std::string source);
  std::string proto_parse_enc_wei(const Moe &moe);
  std::string proto_parse_dec_wei(const Moe &moe);
 ...
 public:
  ...
  // newly added for MoE
  int _expert_num_encoder;
  int _expert_num_decoder;
  int _moe_topk_encoder;
  int _moe_topk_decoder;
  int _n_moelayer_encoder;
