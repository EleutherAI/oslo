...
*/
template <> float GptWeight<OperationType::FP32>::float2required(float value) {
  return value;
}
...
*/
template <> __half GptWeight<OperationType::FP16>::float2required(float value) {
...
template <OperationType OpType_>
std::string
GptWeight<OpType_>::proto_parse_emb_wei(const GptEmbeddingLayer &layer) {
