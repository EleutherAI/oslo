...
*/
template <OperationType OpType_> class QuantVitWeight {
private:
  ...
  void proto_get_model_config(const QuantVit &vit);
  std::string proto_parse_emb_wei(const QuantVitEmbeddingLayer &layer);
  std::string proto_parse_enc_wei(const QuantVit &vit);
  ...
  // store the emb weights on gpu memory
  thrust::device_vector<_DataType> _d_src_emb_wei;
  // store the enc weights on cpu memory
