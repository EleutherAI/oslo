...
void VitWeight<OpType_>::proto_get_model_config(const Vit &vit) {
  _hidden_size = vit.src_embedding().cls_embedding_size();
  _inner_size = vit.encoder_stack()[0].ffn_first_kernel_size() / _hidden_size;
...
*/
template <OperationType OpType_>
std::string VitWeight<OpType_>::proto_parse_enc_wei(const Vit &vit) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;
...
*/
template <OperationType OpType_>
void VitWeight<OpType_>::hdf5_parse_enc_wei(hid_t hdf5_file) {
  size_t value_size =
...
*/
template <OperationType OpType_>
std::string VitWeight<OpType_>::initializing(std::string weight_path) {
