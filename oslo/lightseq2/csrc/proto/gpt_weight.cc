...
*/
template <> float GptWeight<float>::float2required(float value) {
  return value;
}
...
*/
template <> __half GptWeight<__half>::float2required(float value) {
...
*/
template <typename T>
void GptWeight<T>::proto_get_model_config(const Gpt &gpt) {
...
*/
template <typename T>
std::string GptWeight<T>::proto_parse_enc_wei(const Gpt &gpt) {
  std::vector<int> offset;
  std::vector<float> value;
  int idx = 0;
...
*/
template <typename T>
void GptWeight<T>::hdf5_get_model_config(hid_t hdf5_file) {
...
Load the weights of embedding layer into GPU memory.
*/
template <typename T> void GptWeight<T>::hdf5_parse_emb_wei(hid_t hdf5_file) {
...
Load the weights of encoder into GPU memory.
*/
template <typename T> void GptWeight<T>::hdf5_parse_enc_wei(hid_t hdf5_file) {
  size_t value_size =
...
*/
template <typename T>
std::string GptWeight<T>::initializing(std::string weight_path) {
