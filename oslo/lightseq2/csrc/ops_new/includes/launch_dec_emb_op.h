...
// dropout inside ffn.
template <typename T> class LaunchDecEmbOp : public Operator {
private:
...
public:
  LaunchDecEmbOp(int max_batch_tokens, size_t beam_size, size_t hidden_size,
      ...
      : Operator("LaunchDecEmbOp"), _max_batch_tokens(max_batch_tokens),
        _beam_size(beam_size), _hidden_size(hidden_size),
        _trg_vocab_size(trg_vocab_size), _max_step(max_step),
  ...
  void before_forward(size_t batch_size, int cur_step) {
    ...
    _result->set_shape(
        {batch_size, size_t(cur_step + 1), _beam_size, _hidden_size});
  }
