...
template <typename T1, typename T2> class LinearOp : public Operator {
private:
  size_t _output_size;
  size_t _input_size;
...
public:
  LinearOp(size_t max_batch_tokens, size_t output_size, size_t input_size,
           ...
           float alpha = float(1.))
      : Operator("LinearOp"), _max_batch_tokens(max_batch_tokens),
        _output_size(output_size), _input_size(input_size), _opA(opA),
        _opB(opB), _gemm_algos(std::array<int, 3>({99, 99, 99})),
        _alpha(alpha) {}
