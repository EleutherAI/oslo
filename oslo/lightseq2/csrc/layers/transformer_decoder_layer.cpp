...
void TransformerDecoderLayer<T>::encdec_kv_fw(const T *enc_output_ptr) {
  ...
  if (_enable_quant) {
    launch_fake_quantize<T>(nullptr, nullptr, _shared_encdec_kv_ptr,
                            _encdec_attn_kvw_ptr, _encdec_kv_cmax_ptr + 1,
...
void TransformerDecoderLayer<T>::encdec_attn_layer_fw(const T *input_ptr,
  ...
  if (_enable_quant) {
    ...
    cublasLtMatmulAlgo_info encdec_q_algo_info =
        _algo_map.getAlgo(_batch_tokens, _hidden_size, _hidden_size);
    std::vector<LSLayout> encdec_q_layout =
        getLSLayout(encdec_q_algo_info.dataOrder);
    ...
    _encdec_q_linear.Forward(
        _batch_tokens, qin_ptr, qweight_ptr, _igemm_alpha_ptr, _igemm_beta_ptr,
        qout_ptr, _cublasLtHandle, _stream, encdec_q_algo_info, _algo_map);
    ...
    launch_quant_bias_add_transform_20314<T>(
        ...
        _trg_seq_len, 1, _heads, _hidden_size / _heads, _stream, nullptr,
        encdec_q_layout[2] == kCol32);
  ...
  if (_enable_quant) {
    ...
    cublasLtMatmulAlgo_info encdec_attn_algo_info =
        _algo_map.getAlgo(_batch_tokens, _hidden_size, _hidden_size);
    std::vector<LSLayout> encdec_attn_layout =
        getLSLayout(encdec_attn_algo_info.dataOrder);
...
template <typename T> void TransformerDecoderLayer<T>::zero_mask_grad() {
  ...
  _encdec_attn_prob_dropout.zero_mask(_stream);
  _encdec_attn_dropout.zero_mask(_stream);
  _ffn_activation_dropout.zero_mask(_stream);
...
template <typename T>
int8_t *TransformerDecoderLayer<T>::_shared_quant_mem_ptr = nullptr;
