...
from csrc.pytorch.pytorch_quantization.tensor_quant import (
    QuantDescriptor,
    tensor_quant,
    fake_tensor_quant,
)
from csrc.pytorch.pytorch_quantization.nn.modules.clip import Clip

from csrc.pytorch.pytorch_quantization import calib

import csrc.pytorch.pytorch_quantization.utils as quant_utils
...
class TensorQuantizer(nn.Module):
    ...
    Args:
        quant_desc: An instance of :func:`QuantDescriptor <csrc.pytorch.pytorch_quantization.tensor_quant.QuantDescriptor>`.
        ...
        if_calib: A boolean. If True, run calibration. Not implemented yet. Settings of calibration will probably
            go to :func:`QuantDescriptor <csrc.pytorch.pytorch_quantization.tensor_quant.QuantDescriptor>`.
    ...
    def load_calib_amax(self, *args, **kwargs):
        ...
        *args and **kwargs are directly passed to compute_amax, except "strict" in kwargs. Refer to
        ...
        if calib_amax is None:
            err_msg = (
                "Calibrator returned None. This usually happens when calibrator hasn't"
                " seen any tensor."
            )
            if not strict:
