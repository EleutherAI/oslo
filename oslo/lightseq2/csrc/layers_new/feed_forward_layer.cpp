...
FeedForwardLayer<T1, T2>::FeedForwardLayer(
    ...
    : Layer("FeedForwardLayer"), _layer_id(layer_id),
      _max_batch_tokens(max_batch_tokens), _max_seq_len(max_seq_len),
      _hidden_size(hidden_size), _heads(num_heads),
      _intermediate_size(intermediate_size), _is_pre_ln(is_pre_ln),
...
template <typename T1, typename T2>
size_t
FeedForwardLayer<T1, T2>::load_para_and_grad(const T1 *para_ptr,
