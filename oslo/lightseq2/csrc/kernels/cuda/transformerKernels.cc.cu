...
it will be call once on encoder output
...
__global__ void
ker_topk_sample(const T *logits, const T *logit_bias, int *old_input_ids,
                int *new_input_ids, const int vocab_size, const int prompt_len,
                const int max_step, const int batch_seq_len, int logits_seq_len,
                int *unfinished, curandState *curandstate, int eos_id) {
  int last_token_idx_in_batch = blockIdx.x * max_step + batch_seq_len - 1;

  /* add EOS to end if last token is EOS */
  if (batch_seq_len > prompt_len &&
      old_input_ids[last_token_idx_in_batch] == eos_id) {
    if (threadIdx.x == 0) {
...
void ker_topk_sample_launcher(int batch_size, int batch_seq_len,
  ...
  if (k == 1)
    ker_topk_sample<T, 1><<<batch_size, max_thread_per_block, 0, stream>>>(
        logits, logit_bias, old_input_ids, new_input_ids, vocab_size,
        prompt_len, max_step, batch_seq_len, logits_seq_len, unfinished,
        curandstate, eos_id);
  else if (k == 2)
...
template void ker_topk_sample_launcher<float>(
    int batch_size, int batch_seq_len, const int prompt_len, const int max_step,
    int logits_seq_len, int max_thread_per_block, cudaStream_t stream,
    const float *logits, const float *logit_bias, int *old_input_ids,
    int *new_input_idx, const int vocab_size, const int k, int *unfinished,
    curandState *curandstate, int eos_id);

template void ker_topk_sample_launcher<__half>(
    int batch_size, int batch_seq_len, const int prompt_len, const int max_step,
    int logits_seq_len, int max_thread_per_block, cudaStream_t stream,
    const __half *logits, const __half *logit_bias, int *old_input_ids,
    int *new_input_idx, const int vocab_size, const int k, int *unfinished,
    curandState *curandstate, int eos_id);
...
__global__ void ker_topp_sample(const T *logits, const T *logit_bias,
                                ...
                                const int vocab_size, const int prompt_len,
                                const int max_step, const int batch_seq_len,
  ...
  /* add EOS to end if last token is EOS */
  if (batch_seq_len > prompt_len &&
      old_input_ids[token_idx_in_batch] == eos_id) {
    if (threadIdx.x == 0) {
...
void ker_topp_sample_launcher(int batch_size, int batch_seq_len,
  ...
  ker_topp_sample<T><<<batch_size, max_thread_per_block, 0, stream>>>(
      logits, logit_bias, old_input_ids, new_input_ids, vocab_size, prompt_len,
      max_step, batch_seq_len, logits_seq_len, unfinished, p, curandstate,
      eos_id);
}
...
template void ker_topp_sample_launcher<float>(
    ...
    int logits_seq_len, int max_thread_per_block, cudaStream_t stream,
    const float *logits, const float *logit_bias, int *old_input_ids,
    int *new_input_idx, const int vocab_size, const float p, int *unfinished,
    curandState *curandstate, int eos_id);
