...
template <typename T>
__global__ void
kernel_gpt_padding(const T *token_emb, const T *pos_emb, const int *token_ids,
                   T *output, T *pad_mask_ptr, int *left_pad_len_ptr,
                   int batch_size, int beam_size, int seq_len, int hidden_dim,
...
template <typename T>
__global__ void
kernel_gpt_embedding(const T *token_emb, const T *pos_emb, const int *token_ids,
                     T *output, T *pad_mask_ptr, int *left_pad_len_ptr,
                     int batch_size, int beam_size, int seq_len, int hidden_dim,
                     int padding_id, int max_step, int step_offset) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (token_id != padding_id) {
    ...
    float4 pos_emb_val =
        ((float4 *)pos_emb)[(seq_idx + step_offset -
...
__global__ void ker_arrange_qkv_with_cache(const T *ori_qkv, const T *qkv_bias,
  ...
  if (token_id < batch_seq_len - 1) {
    ...
    if (blockIdx.y == 0)
      return;
    if (blockIdx.y == 1)
      new_val = k_cache[old_target_id];
    if (blockIdx.y == 2)
      new_val = v_cache[old_target_id];
  } else {
  ...
  if (blockIdx.y == 0)
    new_q[target_id] = new_val;
  if (blockIdx.y == 1)
    new_k[target_id] = new_val;
  if (blockIdx.y == 2)
...
__global__ void ker_arrange_qkv_with_cache<__half>(
  ...
  if (token_id < batch_seq_len - 1) {
    ...
    if (blockIdx.y == 0)
      return;
    if (blockIdx.y == 1)
      new_val = p_k_cache[old_target_id];
    if (blockIdx.y == 2)
      new_val = p_v_cache[old_target_id];
  } else {
    new_val =
  ...
  if (blockIdx.y == 0)
    p_new_q[target_id] = new_val;
  if (blockIdx.y == 1)
    p_new_k[target_id] = new_val;
  if (blockIdx.y == 2)
    p_new_v[target_id] = new_val;
}
...
template void
ker_ppl_launcher<__half>(int batch_size, int batch_seq_len,
                         int max_thread_per_block, cudaStream_t stream,
                         const __half *logits, const int *input_ids,
                         const int *real_seq_len, float *ppl, int vocab_size);
