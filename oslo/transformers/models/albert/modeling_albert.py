...
class AlbertPreTrainedModel(PreTrainedModel, OsloModel):
    """
...
class AlbertAttention(nn.Module):
    def __init__(self, config: AlbertConfig):
        ...
        self.query = onn.Linear(config.hidden_size, self.all_head_size)
        self.key = onn.Linear(config.hidden_size, self.all_head_size)
        self.value = onn.Linear(config.hidden_size, self.all_head_size)

        self.attention_dropout = nn.Dropout(config.attention_probs_dropout_prob)
        self.output_dropout = onn.FusedBiasDropout(config.hidden_dropout_prob)
    ...
    def forward(
        ...
        projected_context_layer, bias = self.dense(context_layer)
        projected_context_layer_dropout = self.output_dropout(
            projected_context_layer, bias
        )
...
class AlbertLayer(nn.Module):
    ...
    def ff_chunk(self, attention_output: torch.Tensor) -> torch.Tensor:
        ffn_output, bias = self.ffn(attention_output)
        ffn_output = self.activation(ffn_output, bias)
...
class AlbertMLMHead(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.activation(hidden_states, bias)
