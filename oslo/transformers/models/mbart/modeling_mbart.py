...
class MBartPreTrainedModel(PreTrainedModel, OsloModel):
...
# Copied from oslo.transformers.models.bart.modeling_bart._make_causal_mask
...
# Copied from oslo.transformers.models.bart.modeling_bart._expand_mask
...
# Copied from oslo.transformers.models.bart.modeling_bart.BartLearnedPositionalEmbedding with Bart->MBart
...
# Copied from oslo.transformers.models.bart.modeling_bart.BartAttention with Bart->MBart
class MBartAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = onn.Linear(embed_dim, embed_dim, bias=bias)
        self.v_proj = onn.Linear(embed_dim, embed_dim, bias=bias)
        self.q_proj = onn.Linear(embed_dim, embed_dim, bias=bias)
        self.out_proj = onn.Linear(embed_dim, embed_dim, bias=bias)
...
class MBartEncoderLayer(nn.Module):
    def __init__(self, config: MBartConfig):
        ...
        self.activation_fn = F.fused_bias_gelu
        self.activation_dropout = config.activation_dropout
        self.fc1 = onn.Linear(
            self.embed_dim, config.encoder_ffn_dim, skip_bias_add=True
        )
        self.fc2 = onn.Linear(
            config.encoder_ffn_dim, self.embed_dim, skip_bias_add=True
        )
    ...
    def forward(
        ...
        hidden_states, bias = self.fc2(hidden_states)
        hidden_states = F.fused_bias_dropout(
            hidden_states, bias, p=self.dropout, training=self.training, inplace=False
        )
        hidden_states = residual + hidden_states
...
class MBartDecoderLayer(nn.Module):
    def __init__(self, config: MBartConfig):
        ...
        self.fc1 = onn.Linear(
            self.embed_dim, config.decoder_ffn_dim, skip_bias_add=True
        )
        self.fc2 = onn.Linear(
            config.decoder_ffn_dim, self.embed_dim, skip_bias_add=True
        )
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)
...
# Copied from oslo.transformers.models.bart.modeling_bart.BartClassificationHead with Bart->MBart
...
class MBartDecoder(MBartPreTrainedModel):
    ...
    # Copied from oslo.transformers.models.bart.modeling_bart.BartDecoder._prepare_decoder_attention_mask
...
class MBartForConditionalGeneration(MBartPreTrainedModel):
    ...
    def forward(
        ...
        masked_lm_loss = None
        if labels is not None:
            partition_vocab_size = lm_logits.size(-1)
            if (
            ...
            masked_lm_loss = loss_lm(
                lm_logits.view(-1, partition_vocab_size), labels.view(-1)
            )
...
class MBartForSequenceClassification(MBartPreTrainedModel):
    ...
    # Copied from oslo.transformers.models.bart.modeling_bart.BartForSequenceClassification.forward
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
...
class MBartForQuestionAnswering(MBartPreTrainedModel):
    ...
    # Copied from oslo.transformers.models.bart.modeling_bart.BartForQuestionAnswering.forward
    def forward(
        self,
...
# Copied from oslo.transformers.models.bart.modeling_bart.BartDecoderWrapper with Bart->MBart
...
# Copied from oslo.transformers.models.bart.modeling_bart.BartForCausalLM with Bart->MBart, facebook/bart-base->facebook/mbart-large-cc25
